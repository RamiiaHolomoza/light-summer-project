...
const active = document.querySelector(
  '.info-content-item.is-active .accordion-content'
);
...
swiperAbout.on('init', updateActiveElement);
swiperAbout.on('slideChange', updateActiveElement);

function updateActiveElement() {
  if (!swiperAbout || !swiperAbout.slides) return;

  swiperAbout.slides.forEach(slide => {
    slide.classList.remove('swiper-slide-active');
  });

  const activeSlide = swiperAbout.slides[swiperAbout.activeIndex];
  if (activeSlide) {
